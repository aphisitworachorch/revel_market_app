    ...
    render() {
        ...
        if (this.state.loading) {
            ...
            if (this.state.alert == 'network-failed') {
                ...
                display_data.push(
                    <View style={{ backgroundColor: "#2a76b8"}}>
                        <ImageBackground
                            resizeMode="cover"
                            source={{ uri: GOBALS.URL + this.state.shop.shop_image_name }}
                            style={{ height: 280, justifyContent: "flex-end", }}
                        >
                            <View style={{ height: 64, padding: 18, justifyContent: "center", backgroundColor: '#0F4C81', }}>
                                <Text numberOfLines={1} style={[ styles.text_font, { fontSize: 20, color: '#ffff', fontWeight: 'bold' }]}>{this.state.shop.shop_name}</Text>
                            </View>
                        </ImageBackground>
